...
2. По каждому кандидату из `candidates` прочитай задачу (`backlog show <ID>`), затем код по `path`, если
   он есть в улике (у `duplicate` и `no-source` его нет), и коммиты из улик (`git show <sha>`).
3. Реши:
