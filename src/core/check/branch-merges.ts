...
export async function mergesKnownAtCreation(repo: string, tasks: readonly Task[], facts: RepoFacts, origins: ReadonlyMap<string, string>): Promise<KnownMerges> {
  ...
  for (const task of tasks) {
    ...
    for (const merge of merges) {
      if (await bringsNothingNew(repo, merge, origin, path)) known.set(task.id, (known.get(task.id) ?? new Set()).add(merge.sha));
    }
  }
  return known;
}

async function bringsNothingNew(repo: string, merge: Commit, origin: string, path: string): Promise<boolean> {
  const branchCommits = await runGit(repo, ["rev-list", `${merge.sha}^2`, `^${merge.sha}^1`, `^${origin}`, "--", `:(literal)${path}`]);
