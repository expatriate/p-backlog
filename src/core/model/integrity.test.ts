...
describe("integrityErrors", () => {
  ...
  it("эпик должен существовать и быть эпиком", () => {
    ...
    expect(integrityErrors(makeTask({ id: "SPA-2", epic: "SPA-9" }), [plain])).toEqual(["эпик SPA-9 не найден"]);
    expect(integrityErrors(makeTask({ id: "SPA-2", epic: "SPA-1" }), [plain])).toEqual(["SPA-1 не является эпиком"]);
  });

  it("эпик не вкладывается в эпик", () => {
    const outer = makeTask({ id: "SPA-1", type: "epic" });
    const inner = makeTask({ id: "SPA-2", type: "epic", epic: "SPA-1" });
    expect(integrityErrors(inner, [outer])).toEqual(["эпик не может входить в другой эпик"]);
  });

  it("эпик с дочерними задачами нельзя сделать задачей", () => {
    const epic = makeTask({ id: "SPA-1", type: "epic" });
    const child = makeTask({ id: "SPA-2", epic: "SPA-1" });
    expect(integrityErrors({ ...epic, type: "task" }, [epic, child])).toEqual([
  ...
  it("находит цикл блокеров через другие задачи, используя новую версию кандидата", () => {
    ...
    expect(integrityErrors(cWithCycle, [a, b, c])).toEqual(["цикл блокеров: SPA-3 → SPA-1 → SPA-2 → SPA-3"]);
  });
  ...
  it("не сообщает о цикле, который не проходит через кандидата", () => {
    ...
    expect(integrityErrors(c, [a, b, c])).toEqual([]);
  });
});
...
describe("taskWarnings", () => {
  it("добавляет ненайденные ссылки к нарушениям правил", () => {
    ...
    expect(taskWarnings(task, [task])).toEqual(["SPA-8 не найдена", "TI-9 не найдена", "эпик SPA-7 не найден"]);
  });
});
