...
import { closeOnStopSignal, PID_FILE_ENV, startServer } from "../../server/start";
...
async function runServe(args: string[], io: CliIo): Promise<number> {
  ...
  try {
    const server = await startServer({ root: io.backlogRoot, port, home: io.home, env: io.env, pidFile: io.env[PID_FILE_ENV] });
