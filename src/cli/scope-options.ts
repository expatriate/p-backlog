...
export type Scope = { projectIds: string[]; project?: Project };
...
export function resolveScope(loaded: LoadedBacklog, io: CliIo, values: ScopeValues): Scope | null {
  ...
  if (values["all-projects"] === true) return { projectIds: loaded.projects.map((project) => project.id) };
  const project = requireProject(loaded, io, values.project);
  return project === undefined ? null : { projectIds: [project.id], project };
}
